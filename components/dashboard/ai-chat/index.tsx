...
export default function Chat(props: Props) {
  ...
  const handleTranslate = async () => {
    ...
    if (inputMessage.length > maxCodeLength) {
      alert(
        `Vui lòng nhập dưới ${maxCodeLength} ký tự. Hiện bạn đang có ${inputMessage.length} ký tự.`
      );
      return;
    }
    setOutputCode(' ');
    ...
    if (!response.ok) {
      setLoading(false);
      if (response) {
        alert('Có lỗi khi gọi API. Hãy đảm bảo sử dụng API key hợp lệ.');
      }
      return;
    }
         ...
         model === 'gpt-4-1106-preview' ? '' : ''
          ...
          <div className="mt-5 flex flex-col items-center justify-center md:flex-row">
            <p className="text-center text-xs text-zinc-500 dark:text-white">
              Bản xem trước miễn phí. ChatGPT có thể tạo ra thông tin chưa chính
              xác về người, địa điểm hoặc sự kiện. Hãy kiểm tra lại các thông
              tin quan trọng.
            </p>
          </div>
        </div>
